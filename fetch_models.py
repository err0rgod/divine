...
providers = [
    {
        ...
        "headers": {
            "Authorization": f"Bearer {os.environ.get('BLUESMIND_API_KEY', '')}"
        },
        "parser": lambda r: (
    ...
    },
    {
        ...
        "headers": {
            "Authorization": f"Bearer {os.environ.get('FUTUREPPO_API_KEY', '')}"
        },
        "parser": lambda r: (
            [m["id"] for m in r.json().get("data", [])]
            if r and r.json().get("data")
    ...
    },
    {
        ...
        "headers": {
            "Authorization": f"Bearer {os.environ.get('FORGE_AI_API_KEY', '')}"
        },
        "parser": lambda r: [m["id"] for m in r.json().get("data", [])],
    },
    {
    ...
    },
    {
        ...
        "headers": {
            "Authorization": f"Bearer {os.environ.get('GROQ_API_API_KEY', '')}"
        },
        "parser": lambda r: [m["id"] for m in r.json().get("data", [])],
    },
    {
        ...
        "headers": {
            "Authorization": f"Bearer {os.environ.get('MISTRAL_API_API_KEY', '')}"
        },
        "parser": lambda r: [m["id"] for m in r.json().get("data", [])],
    },
    {
        ...
        "headers": {
            "Authorization": f"Bearer {os.environ.get('COHERE_API_API_KEY', '')}"
        },
        "parser": lambda r: [
            m["name"]
