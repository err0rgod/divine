...
from divine.application.errors import UnknownProviderError
...
from divine.providers.runtime import (
    ProviderRuntime,
...
def test_importing_runtime_does_not_eager_load_other_adapters() -> None:
    ...
    code = (
        "import sys\n"
        "import divine.providers.runtime\n"
        "assert 'divine.providers.open_router' not in sys.modules\n"
    )
    proc = subprocess.run(
...
def test_create_provider_instantiates_each_builtin():
    ...
    with (
        patch("divine.providers.openai_chat.provider.AsyncOpenAI"),
        patch("httpx.AsyncClient"),
        patch(
            "divine.providers.runtime.factory.ProviderRateLimiter",
