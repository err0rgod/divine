...
from divine.providers.failure_policy import (
...
from divine.providers.rate_limit import (
...
async def test_degraded_function_retries_unchanged_request_then_succeeds() -> None:
    ...
    with (
        ...
        patch(
            "divine.providers.rate_limit.asyncio.sleep",
            new_callable=AsyncMock,
        ) as sleep,
    ):
        events = [
...
async def test_degraded_function_exhaustion_is_detailed_redacted_overload() -> None:
    ...
    with (
        ...
        patch("divine.providers.openai_chat.provider.trace_event") as trace,
