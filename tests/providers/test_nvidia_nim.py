...
from divine.core.failures import ExecutionFailure
...
async def test_stream_response_restores_aliased_tool_arguments(nim_provider):
    ...
    mock_chunk = _tool_call_chunk(
        name="Grep",
        arguments=json.dumps({"pattern": "needle", "-A": 2, "_divine_arg_type": "py"}),
    )
    ...
    assert "_divine_arg_A" not in properties
    assert "_divine_arg_type" in properties

    deltas = _input_json_deltas(events)
    assert len(deltas) == 1
    assert json.loads(deltas[0]) == {"pattern": "needle", "-A": 2, "type": "py"}
    assert "_divine_arg_type" not in deltas[0]
...
async def test_stream_response_restores_nested_aliased_tool_arguments(nim_provider):
    ...
    mock_chunk = _tool_call_chunk(
        ...
        arguments=json.dumps(
            {"parent": {"_divine_arg_type": "page_id", "id": "page_123"}}
        ),
    )
