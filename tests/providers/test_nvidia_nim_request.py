...
from divine.config.nim import NimSettings
from divine.core.anthropic import set_if_not_none
from divine.core.anthropic.models import MessagesRequest, Tool
from divine.providers.nvidia_nim.request_options import (
    _set_extra,
)
from divine.providers.nvidia_nim.request_options import (
    build_nim_request_body as build_request_body,
)
from divine.providers.nvidia_nim.retry import (
    clone_body_without_chat_template,
    clone_body_without_reasoning_content,
)
from divine.providers.nvidia_nim.tool_schema import (
...
class TestBuildRequestBody:
    ...
    def test_grep_schema_type_parameter_is_aliased_without_mutating_request(self, req):
        ...
        tool_schema["properties"]["_divine_arg_type"] = {
            "type": "string",
            "description": "Existing safe property that collides with the alias",
        }
        tool_schema["required"] = ["pattern", "-A", "_divine_arg_type"]
        ...
        assert (
            properties["_divine_arg_type"]
            == original_schema["properties"]["_divine_arg_type"]
        )
        assert aliases == {"_divine_arg_type_2": "type"}
        assert properties["_divine_arg_type_2"] == original_schema["properties"]["type"]
        assert "-A" in parameters["required"]
        assert "_divine_arg_type" in parameters["required"]
    ...
    def test_nested_schema_keyword_properties_are_aliased_without_mutating_request(
        ...
        assert parent_properties["_divine_arg_type"] == {
            "type": "string",
            "enum": ["page_id"],
        }
        assert parent["required"] == ["_divine_arg_type", "id"]
        assert aliases == {"_divine_arg_type": "type"}
        assert tool_schema == original_schema

    def test_private_alias_metadata_is_stripped_without_mutating_body(self):
        body = {
            "model": "test",
            NIM_TOOL_ARGUMENT_ALIASES_KEY: {"Grep": {"_divine_arg_A": "-A"}},
        }

        upstream_body = body_without_nim_tool_argument_aliases(body)

        assert NIM_TOOL_ARGUMENT_ALIASES_KEY not in upstream_body
        assert body[NIM_TOOL_ARGUMENT_ALIASES_KEY] == {"Grep": {"_divine_arg_A": "-A"}}
        assert nim_tool_argument_aliases_from_body(body) == {
            "Grep": {"_divine_arg_A": "-A"}
        }
