...
from divine.api.handlers import (
    MessagesHandler,
    ResponsesHandler,
    TokenCountHandler,
)
...
from divine.core.anthropic.models import (
    Message,
    MessagesRequest,
    TokenCountRequest,
)
...
from divine.core.openai_responses import OpenAIResponsesRequest
...
async def test_messages_handler_forces_no_thinking_for_safety_classifier() -> None:
    ...
    with patch("divine.api.handlers.messages.trace_event") as trace_mock:
        response = await handler.create(request)
        assert isinstance(response, StreamingResponse)
    ...
    assert _trace_events(
        trace_mock, "divine.api.optimization.safety_classifier_no_thinking"
    ) == [
        {
            "stage": "routing",
...
async def test_messages_handler_preserves_thinking_for_non_classifier() -> None:
    ...
    assert (
        _trace_events(
            trace_mock,
            "divine.api.optimization.safety_classifier_no_thinking",
        )
        == []
    )
...
async def test_messages_handler_optimization_intercepts_before_provider_execution() -> (
    ...
    with patch(
        "divine.api.handlers.messages.try_optimizations",
...
def test_token_count_handler_routes_and_counts_tokens() -> None:
    ...
    with patch("divine.api.handlers.token_count.trace_event") as trace:
