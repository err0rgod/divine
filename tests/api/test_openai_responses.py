...
def responses_client():
    ...
    app = create_test_app()
    with (
        patch("divine.api.routes.resolve_provider", return_value=provider),
...
def test_create_response_pre_start_provider_error_returns_openai_error() -> None:
    ...
    with (
        patch("divine.api.routes.resolve_provider", return_value=provider),
        patch("divine.api.response_streams.trace_event") as trace,
        TestClient(app) as client,
    ):
        response = client.post(
            "/v1/responses",
...
def test_create_response_stream_preserves_interleaved_reasoning_order() -> None:
    ...
    assert completed["output"][2]["arguments"] == '{"value":"Divine"}'
...
def test_create_response_tool_stream_emits_function_call() -> None:
    ...
    assert call["arguments"] == '{"value":"Divine"}'


def test_create_response_malformed_provider_function_call_fails_stream() -> None:
    provider = FakeProvider(
        _anthropic_tool_stream(partial_json='{"value":"Divine" "bad"}')
    )
    app = create_test_app()
    with (
        patch("divine.api.routes.resolve_provider", return_value=provider),
...
def _anthropic_tool_stream(
    tool_name: str = "echo", partial_json: str = '{"value":"Divine"}'
) -> list[str]:
    return [
        format_sse_event("message_start", {"type": "message_start", "message": {}}),
        format_sse_event(
