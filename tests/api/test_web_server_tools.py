...
import divine.api.web_tools.constants as web_tool_constants
from divine.api.handlers import MessagesHandler
from divine.api.web_tools import egress as web_egress
from divine.api.web_tools.egress import (
    WebFetchEgressPolicy,
    WebFetchEgressViolation,
    enforce_web_fetch_egress,
)
from divine.api.web_tools.outbound import (
    _drain_response_body_capped,
    _read_response_body_capped,
    _run_web_fetch,
)
from divine.api.web_tools.request import is_web_server_tool_request
from divine.api.web_tools.streaming import stream_web_server_tool_response
from divine.application.errors import InvalidRequestError
from divine.application.routing import (
    ModelRouter,
    ResolvedModel,
...
from divine.core.anthropic.stream_contracts import (
    assert_anthropic_stream_contract,
    parse_sse_text,
    text_content,
)
from divine.core.version import package_version
from divine.messaging.event_parser import parse_cli_event
...
def test_web_tool_user_agent_reports_installed_package_version() -> None:
    assert {
        "User-Agent": (f"Mozilla/5.0 compatible; divine/{package_version()}")
...
async def test_service_rejects_forced_server_tool_when_local_handler_is_disabled(
    ...
    """Every provider needs Divine's local handler for forced server tools."""
    settings = Settings()
...
async def test_run_web_fetch_redirect_to_blocked_host_raises():
    ...
    with (
        patch(
            "divine.api.web_tools.outbound.ClientSession",
...
async def test_run_web_fetch_excess_redirects_raises():
    ...
    with (
        patch("divine.api.web_tools.constants._MAX_WEB_FETCH_REDIRECTS", 1),
        patch(
            "divine.api.web_tools.outbound.ClientSession",
            return_value=client_cm,
        ),
...
async def test_streams_web_fetch_error_summary_generic_by_default(monkeypatch):
    ...
    monkeypatch.setattr("divine.api.web_tools.outbound._run_web_fetch", boom)
    request = MessagesRequest(
        model="claude-haiku-4-5-20251001",
        max_tokens=100,
        messages=[
    ...
    with patch("divine.api.web_tools.outbound.logger.warning") as log_warn:
        raw = "".join(
            [
                event
                async for event in stream_web_server_tool_response(
