...
from divine.application.errors import (
    ApplicationUnavailableError,
    InvalidRequestError,
)
from divine.config.settings import Settings
from divine.messaging.transcription import TranscriptionService
from divine.providers.nvidia_nim.client import NvidiaNimProvider
from divine.providers.nvidia_nim.voice import NvidiaNimTranscriber
from divine.runtime.application import (
    ApplicationRuntime,
    startup_failure_message,
    warn_if_process_auth_token,
)
from divine.runtime.asgi import RuntimeASGIApp
from divine.runtime.bootstrap import _create_transcriber, build_asgi_app
from divine.runtime.provider_manager import ProviderRuntimeManager
...
def _redirect_divine_home(monkeypatch, tmp_path):
    home = tmp_path / "home"
...
def test_warn_if_process_auth_token_logs_warning(monkeypatch):
    ...
    with patch("divine.runtime.application.logger.warning") as warning:
...
async def test_runtime_startup_logs_admin_url_without_printed_server_banner():
    ...
    with (
        ...
        patch(
            "divine.runtime.application.messaging_platform_factory.create_messaging_components",
            return_value=None,
        ),
...
def test_application_error_handler_does_not_log_error_message():
    ...
    with patch("divine.api.app.logger.error") as log_error:
...
def test_bootstrap_configures_default_log_and_publishes_only_services(tmp_path):
    ...
    with (
        patch(
            "divine.runtime.bootstrap.server_log_path",
            return_value=log_path,
        ),
        patch("divine.runtime.bootstrap.configure_logging") as configure,
    ):
...
def test_bootstrap_honors_process_log_file_override(monkeypatch, tmp_path):
    ...
    with patch("divine.runtime.bootstrap.configure_logging") as configure:
...
async def test_bootstrap_constructs_isolated_runtime_resource_graphs() -> None:
    ...
    with patch("divine.runtime.bootstrap.configure_logging"):
