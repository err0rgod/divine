...
def test_proxy_auth_requires_canonical_bearer_header():
    ...
    with patch("divine.api.routes.get_token_count", return_value=1):
...
def test_proxy_auth_ignores_conflicting_legacy_headers():
    ...
    with patch("divine.api.routes.get_token_count", return_value=2):
        r = client.post(
            "/v1/messages/count_tokens",
            json=payload,
            headers={
...
def test_anthropic_auth_token_normalizes_configured_whitespace():
    ...
    with patch("divine.api.routes.get_token_count", return_value=3):
        r = client.post(
            "/v1/messages/count_tokens",
            json=payload,
