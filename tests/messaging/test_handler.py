...
from divine.messaging.command_context import ReplyClearResult, StopOutcome
from divine.messaging.models import MessageScope
from divine.messaging.platforms.ports import MessagingStartupNotice
from divine.messaging.session import SessionStore
from divine.messaging.trees import (
    CancellationReason,
    CancellationResult,
    CancellationUiOwner,
    FailureResult,
...
from divine.messaging.trees.transitions import CancellationEffect
from divine.messaging.voice import VoiceCancellationResult
...
async def test_handle_message_turn_trace_always_includes_full_message_text(
    ...
    with (
        ...
        patch("divine.messaging.workflow.trace_event") as trace_mock,
    ):
...
async def test_terminal_close_waits_past_interactive_drain_timeout(
    ...
    monkeypatch.setattr(
        "divine.messaging.trees.manager.CANCEL_TASK_DRAIN_TIMEOUT_S",
        0.01,
    )
...
async def test_session_info_rejects_unregistered_real_session_without_recording_it(
    ...
    from divine.messaging.node_event_pipeline import (
