...
if [ "${{1:-}}" = "tool" ] && [ "${{2:-}}" = "install" ]; then
    ...
    cp "$FAKE_FIXTURES/fcc-command.sh" "$FAKE_TOOL_BIN/divine-server"
    cp "$FAKE_FIXTURES/fcc-command.sh" "$FAKE_TOOL_BIN/divine-claude"
    cp "$FAKE_FIXTURES/fcc-command.sh" "$FAKE_TOOL_BIN/divine-pi"
    if [ "$FAIL_STEP" != "fcc-missing" ]; then
        cp "$FAKE_FIXTURES/fcc-command.sh" "$FAKE_TOOL_BIN/divine-codex"
    fi
...
if [ "$name" = "divine-server" ] && [ "${1:-}" = "--version" ]; then
    echo "divine 3.5.18"
fi
""",
    )
...
def test_install_sh_fresh_install_is_verified(posix_harness: PosixHarness) -> None:
    ...
    assert "Divine Gateway is installed and verified." in result.stdout
    ...
    assert any(
        call.startswith(
            "uv:tool install --force --refresh-package divine "
            "--python 3.14.0 divine @ "
...
def test_install_sh_stops_without_success_on_each_failure(
    ...
    assert "Divine Gateway is installed and verified." not in result.stdout
    forbidden = {
        ...
        "fcc-missing": "divine-server:--version",
    }.get(failure)
...
def test_install_sh_voice_flags_only_change_divine_spec(
    posix_harness: PosixHarness,
) -> None:
...
copy /y "%FAKE_FIXTURES%\fcc-command.cmd" "%FAKE_TOOL_BIN%\divine-server.cmd" >nul
copy /y "%FAKE_FIXTURES%\fcc-command.cmd" "%FAKE_TOOL_BIN%\divine-claude.cmd" >nul
copy /y "%FAKE_FIXTURES%\fcc-command.cmd" "%FAKE_TOOL_BIN%\divine-pi.cmd" >nul
if not "%FAIL_STEP%"=="fcc-missing" copy /y "%FAKE_FIXTURES%\fcc-command.cmd" "%FAKE_TOOL_BIN%\divine-codex.cmd" >nul
exit /b 0
:update_shell
...
def test_install_ps1_voice_flags_only_change_divine_spec(
    powershell_harness: PowerShellHarness,
) -> None:
