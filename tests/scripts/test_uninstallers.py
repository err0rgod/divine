...
if [ "${1:-}" = "tool" ] && [ "${2:-}" = "uninstall" ]; then
    ...
    if [ "$FAIL_STEP" = "missing" ] || [ "$FAIL_STEP" = "stale-entrypoint" ]; then
        echo 'Tool `divine` is not installed' >&2
        exit 2
    fi
    for name in divine-server divine-claude divine-codex divine-pi fcc-init divine; do
        /bin/rm -f "$FAKE_TOOL_BIN/$name"
    done
    echo "Uninstalled divine"
    exit 0
fi
exit 43
""",
...
""",
    ...
    return PosixUninstallHarness(home, bin_dir, tool_bin, divine_home, log, env)
...
def test_uninstall_sh_removes_and_verifies_only_fcc(
    ...
    assert posix_uninstall_harness.calls() == [
        "uv:tool dir --bin",
        "uv:tool uninstall divine",
        f"rm:-rf {posix_uninstall_harness.divine_home}",
    ]
...
if "%FAIL_STEP%"=="missing" echo Tool `divine` is not installed 1>&2 & exit /b 2
if "%FAIL_STEP%"=="stale-entrypoint" echo Tool `divine` is not installed 1>&2 & exit /b 2
...
""",
    ...
    return PowerShellUninstallHarness(
        home, bin_dir, tool_bin, divine_home, log, env, powershell, wrapper
    )
...
def test_uninstall_ps1_removes_and_verifies_only_fcc(
    ...
    assert powershell_uninstall_harness.calls() == [
        "uv:tool dir --bin",
        "uv:tool uninstall divine",
        f"remove:{powershell_uninstall_harness.divine_home}",
    ]
...
def test_readme_uninstall_uses_raw_urls_and_verification_contract() -> None:
    ...
    assert (
        ...
        'err0rgod/divine/main/scripts/uninstall.sh" | sh'
    ) in text
    assert (
        '& ([scriptblock]::Create((irm "https://raw.githubusercontent.com/'
        'err0rgod/divine/main/scripts/uninstall.ps1")))'
    ) in text
    assert "verifies every Divine command is gone" in text
