...
import divine.messaging.session.persistence as persistence_module
from divine.config.admin.persistence import PreparedAdminUpdate
from divine.config.settings import Settings
from divine.messaging.command_context import StopOutcome
from divine.messaging.platforms.ports import (
    InboundMessageHandler,
    MessagingPlatformComponents,
    MessagingStartupNotice,
)
from divine.messaging.session import SessionStore
from divine.messaging.workflow import MessagingWorkflow
from divine.providers.runtime import ProviderRuntime
from divine.runtime.application import ApplicationRuntime
...
async def test_composition_records_runtime_before_workspace_setup() -> None:
    ...
    with (
        patch(
            "divine.runtime.application.os.makedirs",
...
async def test_composition_publishes_startup_notice_after_runtime_and_repair() -> None:
    ...
    with (
        patch(
            "divine.runtime.application.cli_managed.ManagedClaudeSessionManager",
            return_value=cli_manager,
        ) as manager_constructor,
        patch("divine.runtime.application.messaging_session.SessionStore"),
        patch(
            "divine.runtime.application.messaging_workflow_module.MessagingWorkflow",
