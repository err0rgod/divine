...
def _smoke_config(**overrides) -> SmokeConfig:
    values = {
        ...
        "timeout_s": 45.0,
        "prompt": "Reply with exactly: Divine_SMOKE_PONG",
        "claude_bin": "claude",
        "worker_id": "main",
        "settings": _settings(),
    }
...
def test_ollama_cloud_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_MODEL_OLLAMA_CLOUD", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_provider_smoke_models_cover_configured_providers_independent_of_model_mapping(
    ...
    monkeypatch.delenv("Divine_SMOKE_MODEL_DEEPSEEK", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
...
def test_openrouter_provider_smoke_uses_concrete_free_model(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_MODEL_OPEN_ROUTER", raising=False)
    config = _smoke_config(
...
def test_wafer_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_MODEL_WAFER", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_minimax_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_MODEL_MINIMAX", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_cloudflare_provider_configuration_requires_token_and_account(
    ...
    monkeypatch.delenv("Divine_SMOKE_MODEL_CLOUDFLARE", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_vercel_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_MODEL_VERCEL", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_huggingface_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_MODEL_HUGGINGFACE", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_cohere_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_MODEL_COHERE", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_github_models_provider_configuration_uses_token(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_MODEL_GITHUB_MODELS", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_sambanova_provider_configuration_uses_api_key(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_MODEL_SAMBANOVA", raising=False)
    config = _smoke_config(
        settings=_settings(
            model="ollama/llama3.1",
            ollama_base_url="",
...
def test_provider_smoke_model_override_accepts_model_name_without_prefix(
    ...
    monkeypatch.setenv("Divine_SMOKE_MODEL_DEEPSEEK", "deepseek-reasoner")
    config = _smoke_config(
        settings=_settings(
            deepseek_api_key="deepseek-key",
            ollama_base_url="",
    ...
    assert models[0].source == "Divine_SMOKE_MODEL_DEEPSEEK"
...
def test_provider_smoke_model_override_accepts_owner_model_name(
    ...
    monkeypatch.setenv(
        "Divine_SMOKE_MODEL_NVIDIA_NIM", "nvidia/nemotron-3-super-120b-a12b"
    )
    config = _smoke_config(
        settings=_settings(
    ...
    assert models[0].source == "Divine_SMOKE_MODEL_NVIDIA_NIM"


def test_provider_smoke_model_override_rejects_wrong_provider_prefix(
    monkeypatch,
) -> None:
    monkeypatch.setenv("Divine_SMOKE_MODEL_DEEPSEEK", "ollama/llama3.1")
    config = _smoke_config(
        settings=_settings(
            deepseek_api_key="deepseek-key",
            ollama_base_url="",
    ...
    try:
        ...
        assert "Divine_SMOKE_MODEL_DEEPSEEK" in str(exc)
    else:
...
def test_mistral_reasoning_smoke_uses_reasoning_default(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_MODEL_MISTRAL_REASONING", raising=False)
    config = _smoke_config(
...
def test_mistral_reasoning_smoke_accepts_override(monkeypatch) -> None:
    monkeypatch.setenv("Divine_SMOKE_MODEL_MISTRAL_REASONING", "mistral-medium-3-5")
    config = _smoke_config(
        settings=_settings(mistral_api_key="mistral-key", ollama_base_url="")
    )
    ...
    assert model.source == "Divine_SMOKE_MODEL_MISTRAL_REASONING"
...
def test_provider_smoke_collection_params_are_grouped_by_provider(
    ...
    monkeypatch.delenv("Divine_SMOKE_MODEL_DEEPSEEK", raising=False)
    monkeypatch.delenv("Divine_SMOKE_MODEL_NVIDIA_NIM", raising=False)
    config = _smoke_config(
        live=True,
        settings=_settings(
            deepseek_api_key="deepseek-key",
...
def test_provider_smoke_includes_local_provider_when_model_mapping_uses_it(
    ...
    monkeypatch.delenv("Divine_SMOKE_MODEL_OLLAMA", raising=False)
    config = _smoke_config()
...
def test_nvidia_nim_cli_models_override_and_append() -> None:
    refs = nvidia_nim_cli_model_refs(
        {
            "Divine_SMOKE_NIM_MODELS": "z-ai/glm-5.2,nvidia_nim/custom/model",
            "Divine_SMOKE_NIM_EXTRA_MODELS": "moonshotai/kimi-k2.6,z-ai/glm-5.2",
        }
    )
    ...
    assert refs["nvidia_nim/z-ai/glm-5.2"] == "Divine_SMOKE_NIM_MODELS"
    assert refs["nvidia_nim/moonshotai/kimi-k2.6"] == ("Divine_SMOKE_NIM_EXTRA_MODELS")


def test_nvidia_nim_cli_models_reject_empty_override() -> None:
    try:
        nvidia_nim_cli_model_refs({"Divine_SMOKE_NIM_MODELS": " , "})
    except ValueError as exc:
        assert "Divine_SMOKE_NIM_MODELS" in str(exc)
    else:
...
def test_nvidia_nim_cli_models_reject_wrong_provider_prefix() -> None:
    try:
        nvidia_nim_cli_model_refs({"Divine_SMOKE_NIM_MODELS": "open_router/model"})
    except ValueError as exc:
...
def test_smoke_config_returns_nvidia_nim_cli_provider_models(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_NIM_MODELS", raising=False)
    monkeypatch.delenv("Divine_SMOKE_NIM_EXTRA_MODELS", raising=False)
    config = _smoke_config(
        settings=_settings(
    ...
    refs = openrouter_free_cli_model_refs(
        {
            ...
            "Divine_SMOKE_OPENROUTER_FREE_EXTRA_MODELS": (
...
def test_openrouter_free_cli_models_reject_empty_override() -> None:
    try:
        openrouter_free_cli_model_refs({"Divine_SMOKE_OPENROUTER_FREE_MODELS": " , "})
    except ValueError as exc:
        assert "Divine_SMOKE_OPENROUTER_FREE_MODELS" in str(exc)
    else:
...
def test_openrouter_free_cli_models_reject_wrong_provider_prefix() -> None:
    try:
        openrouter_free_cli_model_refs(
            {"Divine_SMOKE_OPENROUTER_FREE_MODELS": "nvidia_nim/model"}
        )
    except ValueError as exc:
...
def test_smoke_config_returns_openrouter_free_cli_provider_models(monkeypatch) -> None:
    monkeypatch.delenv("Divine_SMOKE_OPENROUTER_FREE_MODELS", raising=False)
    monkeypatch.delenv("Divine_SMOKE_OPENROUTER_FREE_EXTRA_MODELS", raising=False)
    config = _smoke_config(
        settings=_settings(
