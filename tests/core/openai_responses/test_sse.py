...
from divine.core.anthropic.stream_contracts import parse_sse_text
from divine.core.anthropic.streaming import format_sse_event
from divine.core.async_iterators import AsyncCloseable
from divine.core.failures import ExecutionFailure, FailureKind
from divine.core.openai_responses import (
    OpenAIResponsesAdapter,
    OpenAIResponsesRequest,
)
from divine.core.openai_responses.anthropic_sse import (
    AnthropicSseEvent,
    iter_sse_events,
)
...
async def test_responses_transform_closes_direct_event_source_on_early_close() -> None:
    ...
    with patch(
        "divine.core.openai_responses.stream.iter_sse_events",
        return_value=events,
    ):
...
async def test_anthropic_tool_stream_converts_to_function_call_item() -> None:
    ...
    assert function_call["arguments"] == '{"value":"Divine"}'


@pytest.mark.asyncio
async def test_anthropic_function_tool_arguments_are_normalized() -> None:
    response = await _completed_response_from_sse(
        _aiter(_anthropic_tool_stream(partial_json='{ "value" : "Divine" }')),
        {"model": "nvidia_nim/test-model", "stream": True},
    )

    assert response["output"][0]["arguments"] == '{"value":"Divine"}'
...
async def test_anthropic_malformed_function_tool_arguments_fail_response() -> None:
    text = await _collect_sse(
        _responses_sse(
            _aiter(_anthropic_tool_stream(partial_json='{"value":"Divine" "bad"}')),
            {"model": "nvidia_nim/test-model", "stream": True},
        )
    )
...
async def test_overlapping_text_and_tool_blocks_keep_reserved_output_indexes() -> None:
    ...
    assert completed["output"][1]["arguments"] == '{"value":"Divine"}'
