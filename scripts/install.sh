...
REPO_ARCHIVE_URL="https://github.com/err0rgod/divine/archive/refs/heads/main.zip"
...
Installs Claude Code, Codex, and Pi if missing, ensures a compatible uv, and installs or updates Divine Gateway.
...
package_spec() {
    ...
    if [ "$include_nim" -eq 1 ] && [ "$include_local" -eq 1 ]; then
        printf 'divine[voice,voice_local] @ %s' "$REPO_ARCHIVE_URL"
    elif [ "$include_nim" -eq 1 ]; then
        printf 'divine[voice] @ %s' "$REPO_ARCHIVE_URL"
    elif [ "$include_local" -eq 1 ]; then
        printf 'divine[voice_local] @ %s' "$REPO_ARCHIVE_URL"
    else
        printf 'divine @ %s' "$REPO_ARCHIVE_URL"
    fi
}
...
install_divine() {
    ...
    if [ -n "$torch_backend" ]; then
        run uv tool install --force --refresh-package divine --python "$PYTHON_VERSION" --torch-backend "$torch_backend" "$spec"
    else
        run uv tool install --force --refresh-package divine --python "$PYTHON_VERSION" "$spec"
    fi
}

configure_and_verify_divine() {
    run uv tool update-shell

    if [ "$dry_run" -eq 1 ]; then
        print_command uv tool dir --bin
        printf '+ verify divine-server, divine-claude, divine-codex, and divine-pi in the uv tool bin directory\n'
        print_command divine-server --version
        return 0
    fi
    ...
    for command_name in divine-server divine-claude divine-codex divine-pi; do
        [ -x "$tool_bin/$command_name" ] || fail "Divine Gateway installation did not create $tool_bin/$command_name."
    done

    run "$tool_bin/divine-server" --version
}
...
step "Installing or updating Divine Gateway"
install_divine

step "Configuring PATH and verifying Divine Gateway"
configure_and_verify_divine

if [ "$dry_run" -eq 1 ]; then
    printf '\nDry run complete. No changes were made.\n'
else
    printf '\nDivine Gateway is installed and verified. Start the proxy with: divine-server\n'
    printf 'Run Claude Code with: divine-claude\n'
    printf 'Run Codex with: divine-codex\n'
    printf 'Run Pi with: divine-pi\n'
fi
