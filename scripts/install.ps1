...
$RepoArchiveUrl = "https://github.com/err0rgod/divine/archive/refs/heads/main.zip"
...
function Get-PackageSpec {
    ...
    if ($includeNim -and $includeLocal) {
        return "divine[voice,voice_local] @ $RepoArchiveUrl"
    }
    if ($includeNim) {
        return "divine[voice] @ $RepoArchiveUrl"
    }
    if ($includeLocal) {
        return "divine[voice_local] @ $RepoArchiveUrl"
    }
    return "divine @ $RepoArchiveUrl"
}
...
function Install-FreeClaudeCode {
    ...
    if (-not $DryRun) {
        $uvCommand = Get-ApplicationCommand "uv"
        if (-not $uvCommand) {
            throw "uv is not available for the Divine Gateway installation."
        }
...
function Configure-AndConfirmFreeClaudeCode {
    if ($DryRun) {
        ...
        Write-Host "+ verify divine-server, divine-claude, divine-codex, and divine-pi in the uv tool bin directory"
        Write-Host "+ divine-server --version"
        return
    }
    ...
    $installedCommands = @{}
    foreach ($commandName in @("divine-server", "divine-claude", "divine-codex", "divine-pi")) {
        $command = Get-ApplicationCommand $commandName
        if (-not $command) {
            throw "Divine Gateway installation did not create '$commandName'."
        }
    ...
    Invoke-NativeCommand -FilePath $installedCommands["divine-server"] -Arguments @("--version")
}
...
Write-Step "Installing or updating Divine Gateway"
Install-FreeClaudeCode

Write-Step "Configuring PATH and verifying Divine Gateway"
...
if ($DryRun) {
    ...
    Write-Host "Divine Gateway is installed and verified. Start the proxy with: divine-server"
    Write-Host "Run Claude Code with: divine-claude"
    Write-Host "Run Codex with: divine-codex"
    Write-Host "Run Pi with: divine-pi"
}
