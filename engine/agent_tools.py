...
from .superpowers import create_file, execute_command, read_url, save_memory, search_web
    ...
    # PDF files
        ...
        if fitz:
            try:
                ...
                return f"Failed to extract PDF: {e!s}"
        else:
    ...
    # Image files
        ...
        if is_multimodal_model:
            ...
            if pytesseract and Image:
                ...
                return (
                    "Multimodal not supported by model and pytesseract not installed."
                )
...
def parse_and_execute_tags(response_text: str) -> dict[str, Any]:
    ...
    search_matches = re.finditer(
        r"<search_web>(.*?)</search_web>", response_text, re.DOTALL
    )
    ...
    cmd_matches = re.finditer(
        r"<execute_cmd>(.*?)</execute_cmd>", response_text, re.DOTALL
    )
    ...
    file_matches = re.finditer(
        r'<create_file\s+path=["\'](.*?)["\']>(.*?)</create_file>',
        response_text,
        re.DOTALL,
    )
    ...
    # 5. Read Local File
    read_file_matches = re.finditer(
        r'<read_file\s+path=["\'](.*?)["\']>.*?</read_file>', response_text, re.DOTALL
    )
    ...
    read_file_self_closing = re.finditer(
        r'<read_file\s+path=["\'](.*?)["\']\s*/?>', response_text
    )
    ...
    # 4. Save Memory
    mem_matches = re.finditer(
        r"<save_memory>(.*?)</save_memory>", response_text, re.DOTALL
    )
    for match in mem_matches:
        fact = match.group(1).strip()
        res = save_memory(fact)
        results["Memory Saved"] = res
