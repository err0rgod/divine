...
- Repository protection should use rulesets: a non-bypassable main integrity ruleset requires pull requests, merge queue, required checks, and blocks direct/force pushes to `main`; a separate review ruleset may allow `err0rgod`/admins to bypass review only.
...
- **Shared utilities**: Put shared Anthropic protocol logic in neutral `src/divine/core/anthropic/` modules. Do not have one provider import from another provider's utils.
...
- `src/divine/api/`, `src/divine/cli/`, `src/divine/config/`, `src/divine/core/`, `src/divine/messaging/`, `src/divine/providers/`
- `src/divine/application/`
- `.env.example`
