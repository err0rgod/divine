"""ForgeAI Anthropic-to-OpenAI chat request policy."""
...
_OMITTED_ATTACHMENT_TEXT = (
    "[attachment omitted: ForgeAI does not support image or document inputs]"
)
_OMITTED_ATTACHMENT_BLOCK = {"type": "text", "text": _OMITTED_ATTACHMENT_TEXT}


def build_forge_ai_request_body(
    request_data: MessagesRequest, *, thinking_enabled: bool
) -> dict:
    """Build a ForgeAI Chat Completions body from an Anthropic request."""
    logger.debug(
        "FORGE_AI_REQUEST: chat build model={} msgs={}",
        request_data.model,
    ...
    _validate_forge_ai_request_dict(data)
    ...
    if thinking_enabled:
        if unsafe_tool_followup:
            logger.debug(
                "FORGE_AI_REQUEST: disabling thinking for tool follow-up without "
                "replayable thinking model={} msgs={} tools={}",
                data.get("model"),
                len(data.get("messages", [])),
                len(data.get("tools", [])),
            )
            _remove_forge_ai_thinking_hints(data)
        elif has_tool_history:
            logger.debug(
                "FORGE_AI_REQUEST: keeping thinking for tool follow-up with "
                "replayable thinking model={} msgs={} tools={}",
                data.get("model"),
                len(data.get("messages", [])),
                len(data.get("tools", [])),
            ...
            logger.debug(
                "FORGE_AI_REQUEST: keeping thinking for initial tool request "
                "model={} msgs={} tools={}",
                data.get("model"),
                len(data.get("messages", [])),
                len(data.get("tools", [])),
    ...
    if "messages" in data:
        data["messages"] = _normalize_tool_result_content(
            sanitize_forge_ai_messages_for_openai(data["messages"])
        )
...
def sanitize_forge_ai_messages_for_openai(messages: Any) -> Any:
    """Keep only ForgeAI-required reasoning history on assistant tool calls."""
...
def _strip_unsupported_attachment_blocks(messages: Any) -> Any:
    ...
    if top_level_dropped or nested_dropped:
        logger.warning(
            "FORGE_AI_REQUEST: stripped unsupported attachment blocks "
            "(top_level={} nested_in_tool_result={} placeholder_tool_results={}). "
            "ForgeAI has no vision/document support; the model will not see this content.",
    ...
    for block in blocks:
        ...
        if btype in _UNSUPPORTED_MESSAGE_BLOCK_TYPES:
            raise InvalidRequestError(
                f"ForgeAI native does not support {btype!r} blocks ({where})."
            )
...
def _validate_forge_ai_request_dict(data: dict[str, Any]) -> None:
    mcp = data.get("mcp_servers")
    if mcp:
        raise InvalidRequestError("ForgeAI does not support mcp_servers on requests.")
    ...
    for tool in data.get("tools") or ():
        ...
        if _is_server_listed_tool(tool):
            raise InvalidRequestError(
                "ForgeAI does not support listed Anthropic server tools "
...
def _remove_forge_ai_thinking_hints(data: dict[str, Any]) -> None:
...
def _downgrade_forced_tool_choice(data: dict[str, Any]) -> None:
    ...
    if tool_choice.get("type") != "tool" or not isinstance(
        ...
        return
    logger.debug(
        "FORGE_AI_REQUEST: downgrading forced tool_choice to auto for unsupported "
...
def _apply_forge_ai_chat_extras(
    body: dict[str, Any], _request_data: MessagesRequest, thinking_enabled: bool
) -> None:
    if not thinking_enabled or body.get("model") == "forge_ai-reasoner":
        return
