...
from divine.cli.launchers.common import preflight_proxy
from divine.cli.process_registry import (
    kill_all_best_effort,
)
from divine.config.env_migrations import (
    explicit_env_file_huggingface_warning,
    migrate_owned_env_files,
)
from divine.config.env_template import load_env_template
from divine.config.paths import (
    config_dir_path,
    legacy_env_paths,
    managed_env_path,
)
...
from divine.runtime.bootstrap import build_asgi_app

SERVER_GRACEFUL_SHUTDOWN_SECONDS = 5


def serve(argv: Sequence[str] | None = None) -> None:
    """Start the FastAPI server (registered as `divine-server` script)."""
...
def init(argv: Sequence[str] | None = None) -> None:
    ...
    if migrated_from is not None:
        ...
        print(
            "Edit it to set your API keys and model preferences, then run: divine-server"
        )
        return
    ...
    print("Edit it to set your API keys and model preferences, then run: divine-server")
...
def _print_version_if_requested(argv: Sequence[str] | None) -> bool:
    ...
    if "--version" not in args:
        return False
    print(f"divine {package_version()}")
    return True
