...
const API_KEY_ENV = "Divine_PI_API_KEY";
const BASE_URL_ENV = "Divine_PI_BASE_URL";
...
function catalogModelIds(payload: unknown): string[] {
	if (!isRecord(payload) || payload.object !== "list" || !Array.isArray(payload.data)) {
		throw new Error("Divine model catalog returned an invalid response shape.");
	}
...
export function projectFccModels(payload: unknown): ProviderModelConfig[] {
	...
	if (models.length === 0) {
		throw new Error("Divine model catalog contains no routable provider models.");
	}
	return models;
}
	...
	try {
		...
		try {
			...
			if (error instanceof Error && error.name === "AbortError") {
				throw new Error(`Divine model catalog timed out after ${CATALOG_TIMEOUT_MS}ms.`);
			}
			const message = error instanceof Error ? error.message : String(error);
			throw new Error(`Could not reach the Divine model catalog: ${message}`);
		}

		if (!response.ok) {
			throw new Error(`Divine model catalog returned HTTP ${response.status}${requestIdSuffix(response)}.`);
		}
		...
		try {
			...
			throw new Error(`Divine model catalog returned invalid JSON${requestIdSuffix(response)}.`);
		}
