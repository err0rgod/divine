...
def launch(argv: Sequence[str] | None = None) -> None:
    """Launch Pi with a process-local Divine Gateway provider."""
    ...
    if not extension_path.is_file():
        print(
            "Divine Gateway's bundled Pi extension is missing. Reinstall Divine.",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
def build_pi_launcher_command(
    *,
    ...
    """Return a Pi session command with ephemeral Divine provider registration."""
...
def pi_binary_is_compatible(binary_path: str) -> bool:
    """Return whether an executable exposes the Pi features Divine requires."""
