"""Installed `divine-codex` launcher."""
...
from divine.cli.proxy_auth import proxy_auth_token
from divine.config.paths import codex_model_catalog_path
from divine.config.server_urls import local_proxy_root_url
...
def launch(argv: Sequence[str] | None = None) -> None:
    """Launch Codex CLI with Divine Gateway proxy configuration."""

    settings = get_settings()
    proxy_root_url = local_proxy_root_url(settings)
    if error := preflight_proxy(proxy_root_url):
        print(
            f"Divine Gateway proxy is not reachable at {proxy_root_url}: {error}",
            file=sys.stderr,
        )
        print("Start it in another terminal with: divine-server", file=sys.stderr)
        raise SystemExit(1)
...
def build_codex_launcher_command(
    *,
    ...
    """Return a Codex command with ephemeral Divine provider config."""
...
def codex_model_catalog_config_args(
    ...
    try:
        ...
        if not isinstance(models, list) or not models:
            print(
                "Divine Gateway warning: Codex model catalog is empty; "
        ...
        print(
            "Divine Gateway warning: could not prepare Codex model catalog "
...
def codex_config_args(*, api_url: str, model: str | None = None) -> list[str]:
    """Return Codex `-c` assignments for the ephemeral Divine provider."""

    args = [
        "-c",
        _toml_assignment("model_provider", "fcc"),
        "-c",
        _toml_assignment("model_providers.fcc.name", "Divine Gateway"),
        "-c",
