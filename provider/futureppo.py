...
KEYS_STRING = os.environ.get("FUTUREPPO_API_KEYS", os.environ.get("FUTUREPPO_API_KEY", ""))
...
def chat(prompt, max_tokens=1024, conversation=None, model=MODEL):
    """Send a message to FuturePPO using round-robin/random API keys."""
    if not KEYS:
        print("Error: No FuturePPO API key configured in .env")
...
def main():
    ...
    try:
        with open('../models.json' if os.path.exists('../models.json') else 'models.json', 'r') as f:
            db = json.load(f)
            avail = db.get("FuturePPO", [])
            if avail:
                print("\nAvailable Models for FuturePPO:")
    ...
    print("=" * 60)
    print(f"  FuturePPO Interactive Chat ({MODEL})")
    print("=" * 60)
