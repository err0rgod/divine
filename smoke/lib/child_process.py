...
(``divine.exe`` in use), causing flaky smoke. The smoke runner is
...
def cmd_divine_init() -> list[str]:
    return [
        python_exe(),
        "-c",
        "from divine.cli.entrypoints import init; init()",
    ]


def cmd_divine_version() -> list[str]:
    return [
        python_exe(),
        "-c",
        (
            "import sys; "
            "sys.argv = ['divine-server', '--version']; "
            "from divine.cli.entrypoints import serve; serve()"
        ),
    ]


def cmd_divine_serve() -> list[str]:
    return [
        python_exe(),
        "-c",
        "from divine.cli.entrypoints import serve; serve()",
