...
def test_telegram_bot_api_permissions(smoke_config: SmokeConfig) -> None:
    ...
    if not chat_id:
        pytest.skip("Divine_SMOKE_TELEGRAM_CHAT_ID or ALLOWED_TELEGRAM_USER_ID required")

    marker = f"Divine smoke {int(time.time())}"
    sent = httpx.post(
...
def test_discord_bot_api_permissions(smoke_config: SmokeConfig) -> None:
    ...
    if not channel_id:
        pytest.skip("Divine_SMOKE_DISCORD_CHANNEL_ID or ALLOWED_DISCORD_CHANNELS required")
...
def test_interactive_inbound_messaging_requires_explicit_mode(
    ...
    if not smoke_config.interactive:
        pytest.skip("set Divine_SMOKE_INTERACTIVE=1 for manual inbound messaging checks")
    pytest.skip(
