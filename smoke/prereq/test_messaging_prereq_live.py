...
def test_telegram_bot_api_permissions(smoke_config: SmokeConfig) -> None:
    ...
    if not chat_id:
        pytest.skip(
            "DIVINE_SMOKE_TELEGRAM_CHAT_ID or ALLOWED_TELEGRAM_USER_ID required"
        )
...
def test_discord_bot_api_permissions(smoke_config: SmokeConfig) -> None:
    ...
    if not channel_id:
        pytest.skip(
            "DIVINE_SMOKE_DISCORD_CHANNEL_ID or ALLOWED_DISCORD_CHANNELS required"
        )
...
def test_interactive_inbound_messaging_requires_explicit_mode(
    ...
    if not smoke_config.interactive:
        pytest.skip(
            "set DIVINE_SMOKE_INTERACTIVE=1 for manual inbound messaging checks"
        )
    pytest.skip(
