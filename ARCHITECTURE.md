...
This document is a maintainer-oriented map of Divine Gateway. It explains the
...
Divine Gateway is a local proxy for agent clients. It accepts Anthropic
...
- [src/divine/application/](src/divine/application/) is the dependency-leaf application boundary. It
...
- [src/divine/api/](src/divine/api/) is the HTTP adapter. It owns the FastAPI app, routes, API product
...
- [src/divine/cli/](src/divine/cli/) owns console entrypoints, client CLI launchers, process/session
  management, and client adapter contracts.
- [src/divine/config/](src/divine/config/) owns settings, provider metadata, filesystem paths,
  logging setup, constants, and provider ID catalogs.
- [src/divine/core/](src/divine/core/) owns provider-neutral protocol logic: wire request and response
...
- [src/divine/messaging/](src/divine/messaging/) owns optional platform adapters, incoming message
  handling, tree queues, transcript rendering, persistence, commands, and voice
  support.
- [src/divine/providers/](src/divine/providers/) owns provider construction, the shared OpenAI-chat
...
- [src/divine/runtime/](src/divine/runtime/) is the process composition root. It owns application
...
`divine.cli.entrypoints` imports
`divine.runtime.bootstrap` because the installed server executable
...
[core/version.py](src/divine/core/version.py) is the sole runtime owner
of the Divine release version. It reads installed distribution metadata for
FastAPI/OpenAPI, Divine-owned CLI `--version` output, and the outbound web-tools
...
their wrapped clients except for Divine-owned ephemeral provider configuration.

The main ownership rule is that Anthropic and Responses protocol schemas and
shared protocol behavior belong in [src/divine/core/](src/divine/core/), while request routing and
provider execution belong in [src/divine/application/](src/divine/application/). Routes use core schemas
...
Divine optimizes for installed user workflows, not internal compatibility. The
...
- `divine-server` and the local Admin UI for configuring supported providers,
  model routing, auth, server tools, messaging, and diagnostics.
- `divine-claude`, Claude Code, and the Anthropic-compatible proxy behavior Claude
...
- `divine-codex`, Codex CLI/extensions, and the streaming OpenAI Responses behavior
...
- `divine-pi`, Pi, and the Anthropic-compatible proxy behavior Pi relies on,
  including an Divine-scoped model catalog, streaming text and reasoning, and tool
  use/results.
...
- [api/handlers/](src/divine/api/handlers/) owns customer-facing API product flows:
  ...
  [application/execution.py](src/divine/application/execution.py) only for shared
  provider resolution, preflight, tracing, token counting, and streaming.
- [providers/openai_chat/](src/divine/providers/openai_chat/) owns the common upstream provider
  ...
  protocol rules belong in [src/divine/core/](src/divine/core/).
- [messaging/workflow.py](src/divine/messaging/workflow.py) coordinates messaging runtime
...
- [config/admin/](src/divine/config/admin/) owns Admin UI config behavior. Keep
...
- `divine-server` and `divine` call `divine.cli.entrypoints:serve`.
- `fcc-init` calls `divine.cli.entrypoints:init`.
- `divine-claude` calls `divine.cli.launchers.claude:launch`.
- `divine-codex` calls `divine.cli.launchers.codex:launch`.
- `divine-pi` calls `divine.cli.launchers.pi:launch`.
...
and [scripts/uninstall.ps1](scripts/uninstall.ps1) remove only the Divine uv tool and always
delete the managed `~/.fcc/` tree from [config/paths.py](src/divine/config/paths.py); they do not remove
...
[cli/entrypoints.py](src/divine/cli/entrypoints.py) starts the FastAPI server with Uvicorn.
...
[runtime/bootstrap.py](src/divine/runtime/bootstrap.py) is the single production composition function. The CLI
...
configures logging, constructs the runtime owners and the configured voice
  ...
  the consumer-owned ports in [application/ports.py](src/divine/application/ports.py); Admin operations retain
  their inbound-adapter port in [api/ports.py](src/divine/api/ports.py).

[api/app.py](src/divine/api/app.py) registers routers and exception
...
[runtime/application.py](src/divine/runtime/application.py) owns process startup and shutdown, optional messaging,
...
[runtime/asgi.py](src/divine/runtime/asgi.py) drives that owner from ASGI lifespan messages and preserves
the concise startup-failure contract.

[runtime/provider_manager.py](src/divine/runtime/provider_manager.py) is the only owner that constructs, publishes,
...
streaming responses bind it to Divine's response owner, which first closes the
...
[config/settings.py](src/divine/config/settings.py) owns the flat Pydantic Settings schema:
...
in [config/env_files.py](src/divine/config/env_files.py) and uses this order:
...
source detection for startup warnings also belongs to `src/divine/config/env_files.py`.

[config/paths.py](src/divine/config/paths.py) defines managed paths:
...
[config/model_refs.py](src/divine/config/model_refs.py) owns provider-prefixed model ref
...
[config/admin/](src/divine/config/admin/) owns the Admin UI config manifest and
managed env writes. Provider credential, local URL, proxy, and display-name
metadata is generated from [config/provider_catalog.py](src/divine/config/provider_catalog.py);
...
separate modules. [api/admin_routes.py](src/divine/api/admin_routes.py) exposes local-only
...
It is packaged as a [src/divine/config/](src/divine/config/) resource for `fcc-init` and Admin UI
...
[api/routes.py](src/divine/api/routes.py) exposes the public proxy routes:
...
Admin routes live beside these in [api/admin_routes.py](src/divine/api/admin_routes.py).

Authentication is handled by `require_proxy_auth()` in
[api/dependencies.py](src/divine/api/dependencies.py). If `ANTHROPIC_AUTH_TOKEN` is blank,
proxy auth is disabled. Otherwise Divine accepts exactly `Authorization: Bearer
...
opaque Divine request ID before routing, places it in log context and request state,
...
[api/handlers/](src/divine/api/handlers/) owns the public API product flows.
...
[application/execution.py](src/divine/application/execution.py). `ProviderExecutor` resolves the narrow
...
object.
[api/response_streams.py](src/divine/api/response_streams.py) owns public streaming egress
commit timing. It waits for the first protocol chunk before returning a
successful Divine-owned `StreamingResponse`. Its explicit replay iterator owns the
...
response. Once Divine has finalized the failure, the response includes
`x-should-retry: false` so Divine retains ownership of upstream retry/recovery
...
[src/divine/core/openai_responses/adapter.py](src/divine/core/openai_responses/adapter.py). The adapter
...
[application/routing.py](src/divine/application/routing.py) resolves incoming client model names.
It supports two forms:

- Direct provider model refs such as `nvidia_nim/nvidia/model-name`.
- Gateway model IDs decoded by [core/gateway_model_ids.py](src/divine/core/gateway_model_ids.py).
...
Codex-specific model picker shaping stays out of this route. `divine-codex` fetches
the same `/v1/models` response at launch, converts Divine gateway IDs into
...
`/model`; Divine does not implement a proxy-level `/models` alias.

## Provider Architecture

Provider metadata is neutral and centralized in
[config/provider_catalog.py](src/divine/config/provider_catalog.py). Each
...
[providers/runtime/](src/divine/providers/runtime/) owns construction details for one
...
cleanup. [providers/runtime/factory.py](src/divine/providers/runtime/factory.py)
...
[application/model_metadata.py](src/divine/application/model_metadata.py) owns the immutable
...
[application/ports.py](src/divine/application/ports.py) defines the two provider operations consumed by request
...
[providers/base.py](src/divine/providers/base.py) defines provider-internal construction and lifecycle contracts:
...
[providers/openai_chat/](src/divine/providers/openai_chat/) implements the concrete
...
[providers/mistral/reasoning.py](src/divine/providers/mistral/reasoning.py), including its
...
downgrade: if an upstream NIM deployment rejects explicit budget control, Divine
...
1. Add provider metadata to [config/provider_catalog.py](src/divine/config/provider_catalog.py).
2. Add credentials and related settings to [config/settings.py](src/divine/config/settings.py)
...
3. Let Admin UI provider credential, local URL, and proxy fields come from the
   ...
   [config/admin/](src/divine/config/admin/) only when the generated manifest is
   insufficient.
4. Add an `OpenAIChatProfile` under [providers/openai_chat/](src/divine/providers/openai_chat/) when
...
[src/divine/core/anthropic/](src/divine/core/anthropic/) owns Anthropic-side protocol behavior:
...
- stream lifecycle through `src/divine/core/anthropic/streaming`, including the neutral
...
[src/divine/core/anthropic/streaming/](src/divine/core/anthropic/streaming/). The shared layer owns the
...
[core/failures.py](src/divine/core/failures.py) defines the immutable,
...
[core/diagnostics.py](src/divine/core/diagnostics.py) owns bounded error
...
[providers/failure_policy.py](src/divine/providers/failure_policy.py)
...
[providers/stream_recovery.py](src/divine/providers/stream_recovery.py)
...
records provider-policy eligibility; it never tells the client to retry after Divine
...
[src/divine/core/openai_responses/](src/divine/core/openai_responses/) owns OpenAI Responses support:
...
- streaming-only `/v1/responses` support for Codex/Divine workflows;
...
Divine accepts omitted `stream` or `stream: true`; `stream: false` is rejected with
an OpenAI-shaped client error because installed Divine/Codex workflows only need
...
entrypoint;
[src/divine/core/openai_responses/streaming/](src/divine/core/openai_responses/streaming/) owns the
...
Divine does not validate custom-tool grammars.
...
`encrypted_content`; the opaque value is not exposed as visible text and Divine
...
[api/optimization_handlers.py](src/divine/api/optimization_handlers.py) short-circuits
...
[api/web_tools/](src/divine/api/web_tools/). When `ENABLE_WEB_SERVER_TOOLS` is true, the
Messages handler can stream local Anthropic server-tool responses without sending the
request upstream. [api/web_tools/egress.py](src/divine/api/web_tools/egress.py) enforces URL
...
[cli/proxy_auth.py](src/divine/cli/proxy_auth.py) owns the neutral
...
while Divine continues to run without API authentication.

[cli/claude_env.py](src/divine/cli/claude_env.py) owns the canonical
Claude Code proxy environment used by every Divine-launched Claude process. It
...
[cli/launchers/claude.py](src/divine/cli/launchers/claude.py) owns the installed
`divine-claude` launcher:

- `divine-claude` applies the shared proxy environment without changing the user's
  Claude command arguments.

[cli/launchers/codex.py](src/divine/cli/launchers/codex.py) owns the installed
`divine-codex` launcher:

- `divine-codex` strips official OpenAI and Codex credential variables.
...
[cli/launchers/pi.py](src/divine/cli/launchers/pi.py) owns the installed
`divine-pi` launcher and [cli/launchers/pi_extension.ts](src/divine/cli/launchers/pi_extension.ts)
is its bundled Pi adapter:

- Session commands load the extension from its absolute installed path and
  scope Pi to the ephemeral `divine/**` provider, whose model IDs
  retain Divine's nested `provider/model` routing reference.
- The extension fetches Divine's `/v1/models` catalog before registration, projects
...
[cli/managed/](src/divine/cli/managed/) owns managed Claude Code subprocesses used by
...
Codex and Pi are supported through their installed launchers. Divine does not keep
...
Messaging is optional. [runtime/application.py](src/divine/runtime/application.py) calls
`create_messaging_components()` from
[messaging/platforms/factory.py](src/divine/messaging/platforms/factory.py) during startup.
...
[messaging/platforms/ports.py](src/divine/messaging/platforms/ports.py): a
...
Runtime adapters in
[messaging/platforms/telegram.py](src/divine/messaging/platforms/telegram.py) and
[messaging/platforms/discord.py](src/divine/messaging/platforms/discord.py) own SDK client
...
[messaging/platforms/telegram_inbound.py](src/divine/messaging/platforms/telegram_inbound.py)
and [messaging/platforms/discord_inbound.py](src/divine/messaging/platforms/discord_inbound.py).
Outbound SDK calls live in
[messaging/platforms/telegram_io.py](src/divine/messaging/platforms/telegram_io.py) and
[messaging/platforms/discord_io.py](src/divine/messaging/platforms/discord_io.py). Shared
delivery policy lives in [messaging/platforms/outbox.py](src/divine/messaging/platforms/outbox.py),
...
[messaging/platforms/voice_flow.py](src/divine/messaging/platforms/voice_flow.py), which owns
...
in the `PendingVoiceRegistry` owned by [messaging/voice.py](src/divine/messaging/voice.py).
...
[messaging/workflow.py](src/divine/messaging/workflow.py) contains `MessagingWorkflow`, the
...
[messaging/turn_intake.py](src/divine/messaging/turn_intake.py) owns slash command dispatch,
...
[messaging/node_runner.py](src/divine/messaging/node_runner.py) owns managed CLI session
...
[messaging/event_parser.py](src/divine/messaging/event_parser.py) normalizes managed Claude
JSON events into low-level transcript events.
[messaging/transcript/](src/divine/messaging/transcript/) owns transcript assembly and
...
[messaging/rendering/](src/divine/messaging/rendering/).

[messaging/command_context.py](src/divine/messaging/command_context.py) defines the typed
...
[messaging/trees/runtime.py](src/divine/messaging/trees/runtime.py) contains the
...
`parent_reference_id` records the exact prompt or Divine status that received the
...
[messaging/trees/transitions.py](src/divine/messaging/trees/transitions.py) owns frozen,
...
[messaging/trees/manager.py](src/divine/messaging/trees/manager.py) is the only external
...
[messaging/trees/repository.py](src/divine/messaging/trees/repository.py)
is manager-private and owns only aggregate/reference indexes.
[messaging/trees/processor.py](src/divine/messaging/trees/processor.py) owns every
...
[messaging/trees/node.py](src/divine/messaging/trees/node.py) owns
...
[messaging/trees/graph.py](src/divine/messaging/trees/graph.py) owns parent/child and
status-message lookup state, and
[messaging/trees/snapshot.py](src/divine/messaging/trees/snapshot.py) owns typed persisted
...
[messaging/session/](src/divine/messaging/session/) persists typed conversation snapshots
...
[messaging/session/persistence.py](src/divine/messaging/session/persistence.py). One writer
...
ownership lives in
[messaging/session/managed_message_log.py](src/divine/messaging/session/managed_message_log.py).
The registry accepts managed inbound prompts, voice notes, and commands as well
as Divine output. It migrates legacy `message_log` entries and persists the final
...
`/clear` commits Divine state cleanup first and then best-effort deletes the exact
authorized message-ID set through the list-based outbound port. Standalone clear
deletes every tracked user and Divine message in its chat; reply clear deletes only
...
missing messages; such failures never restore cleared Divine state.
...
[core/trace.py](src/divine/core/trace.py) emits structured trace events across stages such
...
1. Add or expose the setting in [config/settings.py](src/divine/config/settings.py).
2. Add the template key to [.env.example](.env.example) if users configure it.
3. Add a `ConfigFieldSpec` under [config/admin/](src/divine/config/admin/), or add
...
1. For an installed wrapper, add or update a launcher under
   [cli/launchers/](src/divine/cli/launchers/) and keep credential stripping local to that
   client.
2. For messaging-managed execution, update [cli/managed/](src/divine/cli/managed/) only
...
3. Ensure managed task parsing emits the event shapes expected by
   [messaging/event_parser.py](src/divine/messaging/event_parser.py) and
   [messaging/node_event_pipeline.py](src/divine/messaging/node_event_pipeline.py).
...
1. Implement a `MessagingRuntime`, `OutboundMessenger`, and inbound normalizer
   under [messaging/platforms/](src/divine/messaging/platforms/).
2. Reuse [messaging/platforms/outbox.py](src/divine/messaging/platforms/outbox.py) for
   queued outbound delivery and
   [messaging/platforms/voice_flow.py](src/divine/messaging/platforms/voice_flow.py) for
   voice-note handoff when the platform supports audio.
3. Add construction logic to
   [messaging/platforms/factory.py](src/divine/messaging/platforms/factory.py).
4. Add settings and admin fields for tokens, allowlists, and platform-specific
   runtime options.
5. Add rendering profile support in
   [messaging/rendering/profiles.py](src/divine/messaging/rendering/profiles.py) if needed.
6. Add deterministic runtime/outbound/workflow tests and optional live smoke
   targets.

### Add Protocol Behavior

1. Put shared Anthropic behavior under [src/divine/core/anthropic/](src/divine/core/anthropic/).
2. Put OpenAI Responses behavior under
   [src/divine/core/openai_responses/](src/divine/core/openai_responses/).
