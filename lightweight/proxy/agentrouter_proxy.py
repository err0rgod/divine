...
def load_keys():
    try:
        with open("D:/divine/config/proxy_keys.json") as f:
            data = json.load(f)
...
class AgentRouterProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        try:
            ...
            # Key resolution
            if "MISTRAL_API_KEYS" in globals():
                key = MISTRAL_API_KEYS[0] if MISTRAL_API_KEYS else ""
            elif "AGENTROUTER_API_KEYS" in globals():
                key = AGENTROUTER_API_KEYS[0] if AGENTROUTER_API_KEYS else ""
            else:
                key = API_KEYS[0] if "API_KEYS" in globals() and API_KEYS else ""

            # URL resolution
            if "MISTRAL_API_URL" in globals():
                api_url = MISTRAL_API_URL
            elif "AGENTROUTER_API_URL" in globals():
                api_url = AGENTROUTER_API_URL
            else:
                api_url = API_URL
...
if __name__ == "__main__":
    ...
    with ThreadedHTTPServer(("", PORT), AgentRouterProxyHandler) as httpd:
        print(
            f"AgentRouter Translation Proxy running at http://localhost:{PORT}/v1/messages"
        )
