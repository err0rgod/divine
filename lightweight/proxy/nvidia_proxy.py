...
API_KEYS = [os.environ.get("NVIDIA_NIM_API_KEY")]
DEFAULT_TARGET_MODEL = "meta/llama-3.1-8b-instruct"
PROXY_NAME = "Nvidia"
...
def anthropic_to_openai_request(anthropic_data):
    ...
    if sys_p:
        if isinstance(sys_p, str):
            req["messages"].append({"role": "system", "content": sys_p})
        elif isinstance(sys_p, list):
            req["messages"].append(
                {
                    "role": "system",
                    "content": "\n".join(
                        b.get("text", "") for b in sys_p if b.get("type") == "text"
                    ),
                }
            )
        ...
        for block in content:
            if block["type"] == "text":
                ...
                if isinstance(b_content, str):
                    tool_msg["content"] = b_content
                elif isinstance(b_content, list):
                    tool_msg["content"] = "\n".join(
                        b["text"] for b in b_content if b.get("type") == "text"
                    )
...
class ProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path.endswith("/v1/models"):
            ...
            self.end_headers()
            self.wfile.write(
                b'{"object": "list", "data": [{"id": "claude-3-7-sonnet-20250219", "type": "model"}]}'
            )
        else:
            self.send_response(200)
            self.end_headers()
            self.wfile.write(f"{PROXY_NAME} Proxy is running!".encode())
    ...
    def do_POST(self):
        try:
            ...
            if ": " in target_req.get("model", ""):
                target_req["model"] = target_req["model"].split(": ")[1]
            ...
            key = (
                API_KEYS[0]
                if "API_KEYS" in globals() and API_KEYS and API_KEYS[0]
                else ""
            )
            if "FORGE_API_KEYS" in globals() and FORGE_API_KEYS:
                key = FORGE_API_KEYS[0]  # Fallback for forge
            ...
            response = requests.post(
                api_url, json=target_req, headers=headers, stream=is_stream, timeout=120
            )
